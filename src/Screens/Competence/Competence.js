...
function Competence() {
  ...
  const getTableData = async (flag) => {
    ...
    try {
      ...
      const { data } = await getEmployeeTrainingData(params);
      const { employees, employeesData } = data;

      const newList = employees?.map((d, index) => {
        const departmentName = d.department?.name || "-";
        const departmentId = d.department?._id || "-";
        const startDate = d?.startDate
          ? dayjs(d.startDate).format("DD MMM YYYY")
          : "-";
        const endDate = d?.endDate
          ? dayjs(d.endDate).format("DD MMM YYYY")
          : "-";
        const startTime = d?.startTime
          ? dayjs(d.startTime, "HH:mm").format("hh:mm A")
          : "-";
        const endTime = d?.endTime
          ? dayjs(d.endTime, "HH:mm").format("hh:mm A")
          : "-";
        const trainingIdentifiedDate = d?.trainingIdentifiedDate
          ? dayjs(d.trainingIdentifiedDate, "HH:mm").format("hh:mm A")
          : "-";
        ...
        return {
          ...
          startTime: startTime,
          endTime: endTime,
          trainingIdentifiedDate: trainingIdentifiedDate,
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          <Grid container spacing={1} alignItems="center">
            <Grid item xs={12} sx={4} md={3} lg={3}>
              <TextField
                placeholder="Search"
                variant="outlined"
                size="small"
        ...
        {competencePopup ? (
          <CompetencePopup
            open
            onClose={handleCompetenceClose}
            competencePopup={competencePopup}
